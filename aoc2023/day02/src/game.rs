...
pub struct GameTurn{ pub red: u32, pub green: u32, pub blue: u32 }
...
impl FromStr for GameTurn {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let map: HashMap<String, Result<u32, ParseIntError>> = s.trim().split(',')
        ...
        Ok(GameTurn{ red: red, green: green, blue: blue })
    }
}

#[derive(Debug, PartialEq)]
pub struct Game{ pub id: u32, pub turns: Vec<GameTurn> }

impl Game {
    pub fn max_turn(self) -> GameTurn {
        self.turns.into_iter().reduce(|a, b| GameTurn::max_turn(a, b)).unwrap()
    }
    ...
    pub fn check_game(&self, reference: &GameTurn) -> bool {
        (&self.turns).into_iter()
            .map(|t| t.check_turn(reference)).reduce(|a, b| a && b)
            .unwrap_or(true)
    }
}
...
impl FromStr for Game {
    type Err = Box<dyn error::Error>;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let res = s.trim().split_once(':').map(|(gid, tx)| {
            let id = u32::from_str(&gid.to_lowercase().replace("game ", ""));
            let turns: Vec<GameTurn> = (tx.split(";").map(|t| GameTurn::from_str(t).unwrap())).collect();
            id.map(|i| Game{ id: i, turns: turns })
        }).unwrap();
        Ok(res?)
    }
}